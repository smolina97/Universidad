...
public class FatesScriptController implements Initializable
{
	...
	@FXML private void beginDragDrop(DragEvent event)
	{
	...
	@FXML private void openFile() throws Exception
	{
		FileChooser chooser = new FileChooser();
		FileChooser.ExtensionFilter cmbFilter = new FileChooser.ExtensionFilter("Script file (*.cmb)", "*.cmb");
		FileChooser.ExtensionFilter fscriptFilter = new FileChooser.ExtensionFilter("Uncompiled Script file (*.fscript)", ".fscript");
	...
	@FXML private void saveFile() throws Exception
	{
		if(tabPane.getSelectionModel().getSelectedItem() != null)
		{
			FileChooser chooser = new FileChooser();
			FileChooser.ExtensionFilter extFilter = new FileChooser.ExtensionFilter("Uncompiled Script file (*.fscript)", ".fscript");
	...
	@FXML private void compileAllTabs() throws Exception
	{
		if(tabPane.getTabs().size() > 0)
		{
	        ...
	        if(file != null)
	        {
	        	...
	        	{
	        		ScriptCompiler compiler = new ScriptCompiler(new File(file.getAbsolutePath() + "\\" + tab.getText()), "rw");
	                CodeArea script = (CodeArea) tab.getContent().lookup("#scriptArea");
	                compiler.writeFile(script.getText());
	                compiler.close();
	        	}
	        }	
		}
	...
	@FXML private void createNewScriptTab(DragEvent event) throws Exception
	{
	...
	@FXML private void compileCurrentTab() throws Exception
	{
        if(tabPane.getSelectionModel().getSelectedItem() != null)
        {
        	FileChooser chooser = new FileChooser();
    		FileChooser.ExtensionFilter extFilter = new FileChooser.ExtensionFilter("Script files (*.cmb)", "*.cmb");
            chooser.getExtensionFilters().add(extFilter);
        	chooser.setInitialFileName(tabPane.getSelectionModel().getSelectedItem().getText());
            ...
            if(file != null)
            {
                ScriptCompiler compiler = new ScriptCompiler(file, "rw");
	...
	private void createTab(File input) throws Exception
	{
		...
		if(!setStyle)
		{
			MainData.getInstance().getStage().getScene().getStylesheets().add(this.getClass().getResource("highlighting.css").toExternalForm());
			setStyle = true;
		}
		CodeArea currentScript = (CodeArea) root.lookup("#scriptArea");
		VirtualizedScrollPane<CodeArea> vsPane = new VirtualizedScrollPane<CodeArea>(currentScript);
        ...
        MainData.getInstance().getStage().getScene().widthProperty().addListener(new ChangeListener<Number>() {
        ...
        MainData.getInstance().getStage().getScene().heightProperty().addListener(new ChangeListener<Number>() {
        	@Override public void changed(ObservableValue<? extends Number> observableValue, Number oldSceneWidth, Number newSceneWidth) {
		        if(tabPane.getSelectionModel().getSelectedIndex() != -1)
		        {
		        	try {
        ...
        currentScript.textProperty().addListener(new ChangeListener<Object>() {
            @Override
        ...
        {
        	Decompiler decompiler = new Decompiler(input, "rw");
    		currentScript.appendText(decompiler.getScript());
    		currentScript.autosize();
    		decompiler.close();	
        }
        else if(this.getFileExtension(input).equals("fscript"))
        {
        	currentScript.appendText(readFileAsString(input.getAbsolutePath(), null));
        }
