...
{
	private Hashtable<Integer, Integer> savedLines = new Hashtable<Integer, Integer>();
	private Stack<SimpleEntry<Integer, Integer>> conditionals = new Stack<SimpleEntry<Integer, Integer>>();
	...
	public ArrayList<Byte> createEvent(ScriptCompiler compiler, String[] input, int start, int lineNumber) throws Exception
	{
	...
	public void compileLine(ArrayList<Byte> event, String[] input, int start, ScriptCompiler compiler, int lineNumber) throws Exception
	{
		savedLines.put(start, event.size());
		if(input[start].startsWith("pass") || input[start].startsWith("fail") || input[start].startsWith("specialCheck") || input[start].startsWith("unknownCheck"))
		{
	...
	public ArrayList<Byte> compileParameters(String[] parameters, ScriptCompiler compiler) throws Exception
	{
	...
	public ArrayList<Byte> compileString(String[] input, int start, ScriptCompiler compiler) throws Exception
	{
		String label = ScriptUtils.parseSingleParameter(input[start]);
		ArrayList<Byte> output = new ArrayList<Byte>();
		if(MainData.getInstance().isTranslate() && MainData.getInstance().getEnglishToJapanese().containsKey(label))
		{
			label = MainData.getInstance().getEnglishToJapanese().get(label);
		}
		short offset;
	...
	public void parseEventLine(ArrayList<Byte> event, String[] input, int start, ScriptCompiler compiler) throws Exception
	{
		...
		for(byte b : bytes)
			event.add(b);
		if(MainData.getInstance().getTags().containsKey(label))
		{
			for(byte b : MainData.getInstance().getTags().get(label))
				event.add(b);
			updateConditionals(MainData.getInstance().getTags().get(label).length);
		}
		updateConditionals(3);
	}
	
	public void compileConditional(ArrayList<Byte> event, String[] input, int start) throws Exception
	{
		conditionals.push(new SimpleEntry<Integer, Integer>(event.size(), 2)); // Minimum condition length is 2, so initialize with that value.
	...
	public void compileConditionalEnd(ArrayList<Byte> event, String[] input, int start)
	{
		...
		ArrayList<SimpleEntry<Integer, Integer>> temp = new ArrayList<SimpleEntry<Integer, Integer>>();
	...
	public void compileGoTo(ArrayList<Byte> event, String[] input, int start, int lineNumber)
	{
		int targetLine = Integer.parseInt(ScriptUtils.parseSingleParameter(input[start])) - lineNumber; //Get the target line relative to the array.
