...
import application.utils.dsdecmp.HexInputStream;
...
public class CompressionUtils 
{
	...
	public static byte[] decompress(byte[] bytes) {
		try
		{
			...
			HexInputStream hexStream = new HexInputStream(new ByteArrayInputStream(bytes));
			int[] output = JavaDSDecmp.decompress(hexStream);
			bytes = new byte[output.length];
			for (int i = 0; i < output.length; i++)
				bytes[i] = (byte) output[i];
			return bytes;
